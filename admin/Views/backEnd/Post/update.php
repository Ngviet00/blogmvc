...
<div id="wrapper">
   ...
   <div id="content_main_category" class="bg-light pt-4">
      ...
      <form method="POST" action="?controller=post&action=update_post&id=<?php echo $post['id']; ?>" class="w-75 ml-5 pb-5" enctype='multipart/form-data'>

         <div class="form-group">
            <label for="title" class="pb-1">Tiêu đề:</label>
            <input type="text" required class="form-control" value="<?php echo $post['title'] ?>" name="title" id="title">
         </div>

         <div class="form-group">
            <label for="short_des" class="pb-1">Mô tả ngắn:</label>
            <input type="text" required class="form-control" value="<?php echo $post['short_desc'] ?>" name="short_des" id="short_des">
         </div>

         <div class="form-group">
            <label for="post_content" class="pb-1">Nội dung:</label>
            <textarea required class="form-control" id="post_content" name="post_content" rows="3"><?php echo $post['content'] ?></textarea>
         </div>

         <div class="form-group">
            <label for="createdBy" class="pb-1">Người tạo:</label>
            <input type="text" required disabled class="form-control" value="<?php echo $post['created_by'] ?>" name="createdBy" id="createdBy">
         </div>
         ...
         <div class="input-group mb-3 w-50">
            ...
            <select class="custom-select" id="inputGroupSelect01" name="category_id">
               ...
               ?>
                  <option <?php if ($post['category_id'] == $value['id']) {
                              echo "selected";
                           } ?>>
                     <?php echo $value['name']; ?>
                  </option>
